...
class TakeTutanKhamunSequence(Node):
    def __init__(self):
        super().__init__('take_tutan_khamun_sequence')
...
def main(args=None):
    ...
    try:
        ...
        # Step 2: Move to Tool 1 Take
        node.get_logger().info("=== STEP 2: Moving to Tool 1 Take ===")
        if not node.execute_joint_trajectory(node.tool_1_take, sec=3):
            raise Exception("Step 2 Failed!")
            
        # Let controller settle
        time.sleep(0.5)
        
        # Step 3: Cartesian Slide to Tool 1 Drop
        node.get_logger().info("=== STEP 3: Cartesian Slide to Tool 1 Drop ===")
        if not node.execute_cartesian_to_joints(node.tool_1_drop):
            raise Exception("Step 3 Failed!")
            
        # Step 4: Attach Tool in Simulation
        node.get_logger().info("=== STEP 4: Attaching Tool in Simulation ===")
        msg = String()
        msg.data = 'attach tutankhamun'
