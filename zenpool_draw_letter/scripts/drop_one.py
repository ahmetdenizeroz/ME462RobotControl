...
class DropTutanKhamunSequence(Node):
    def __init__(self):
        super().__init__('drop_tutan_khamun_sequence')
        ...
        self.tool_1_take = [
            -2.386735264454977, -2.239531179467672, -1.259566068649292, 
            -1.213236854677536, 1.5708116292953491, 3.110840082168579
        ]
        
        self.tool_1_drop = [
            -2.402625624333517, -1.7159730396666468, -2.0344910621643066, 
            -0.9617884916118165, 1.570833683013916, 3.0950727462768555
        ]
...
def main(args=None):
    ...
    try:
        ...
        if not node.execute_manhattan_to_joints(node.tool_drop_anchor):
            raise Exception("Step 1 Failed!")
            
        # Step 2: Move to Tool 1 Drop
        node.get_logger().info("=== STEP 2: Moving to Tool 1 Drop ===")
        if not node.execute_joint_trajectory(node.tool_1_drop, sec=3):
        ...
        # Step 4: Cartesian Slide to Tool 1 Take (Robot pushes the physical tool into the slot)
        node.get_logger().info("=== STEP 4: Cartesian Slide to Tool 1 Take ===")
        if not node.execute_cartesian_to_joints(node.tool_1_take):
